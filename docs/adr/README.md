...
| [028-roles-functions-model-and-assignment.md](028-roles-functions-model-and-assignment.md) | Принято (уточнён ADR-029) | Модель «Роли и Функции»; Поручение (конверт+тело, FIPA ACL); Координатор; `docs/assignment/` (планы 1/4/5) |
| [029-assignment-model-refinements.md](029-assignment-model-refinements.md) | Принято | Уточнения: вход=1 Поручение/выход=0..N; состояние=рабочая директория; PRD vs Поручение; периодический HITL; Skill/Agent/Command |
